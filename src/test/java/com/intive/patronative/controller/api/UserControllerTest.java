...
class UserControllerTest {
    ...
    void userAlreadyExists_shouldThrowException() throws Exception {
        ...
        // when
        final var action = mvc.perform(post("/api/users").accept(MediaType.APPLICATION_JSON));
    ...
    void userNotExists_shouldCreated() throws Exception {
        // given
        when(userService.saveUser(any(UserRegistrationRequestDTO.class)))
                .thenReturn(any(UserRegistrationResponseDTO.class));

        // when
        final var action = mvc.perform(post("/api/users")
